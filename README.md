...
```text
Terraform
  -> VPC/EKS/ALB Controller IRSA 생성
Argo CD
...
├── infra
│   ├── platform                 # VPC, EKS, node group, EBS CSI, ALB Controller IRSA, legacy edge support
│   ├── dns                      # Route53 public hosted zone for dannis.cloud
│   ├── edge-frontend            # legacy API Gateway stack, not used by default lifecycle
│   ├── edge-woori               # legacy API Gateway stack, not used by default lifecycle
│   ├── edge-wallet              # legacy API Gateway stack, not used by default lifecycle
│   └── edge-monitoring          # legacy Grafana API Gateway stack, not used by default lifecycle
├── modules/service-edge          # legacy/optional API Gateway edge 공통 Terraform 모듈
├── apps                          # Argo CD가 sync하는 앱/DB Kubernetes manifest
├── addons
│   ├── argocd                   # Argo CD Helm values
│   ├── aws-load-balancer-controller # AWS Load Balancer Controller Helm values
...
| `SERVICE_MODE=platform` | VPC, public/private subnet, NAT Gateway 1개, EKS, node group, EBS CSI, AWS Load Balancer Controller IRSA, legacy API Gateway VPC Link/shared internal ALB support |
| `SERVICE_MODE=dns` | Route53 public hosted zone for `dannis.cloud` |
| `SERVICE_MODE=edge-frontend` | legacy frontend API Gateway stack. 기본 apply/stop/destroy lifecycle에서는 사용하지 않음 |
| `SERVICE_MODE=edge-woori` | legacy woori-backend API Gateway stack. 기본 lifecycle에서는 사용하지 않음 |
| `SERVICE_MODE=edge-wallet` | legacy wallet-backend API Gateway stack. 기본 lifecycle에서는 사용하지 않음 |
| `SERVICE_MODE=edge-monitoring` | legacy Grafana API Gateway stack. 기본 lifecycle에서는 사용하지 않음 |

기본 lifecycle은 `edge-*` 스택을 생성하지 않습니다. 기존 배포에서 남은 edge 리소스는 `make legacy-edges-destroy`로 별도 정리합니다.

## 현재 아키텍처

frontend, wallet-backend, woori-backend, Grafana는 Kubernetes Ingress가 만드는 public ALB 1개로 공개합니다.
...
| 서비스 | 외부 진입 | Kubernetes Service | Backend 연결 |
| --- | --- | --- | --- |
| frontend | AWS Load Balancer Controller Ingress/public ALB | `frontend/frontend` | ClusterIP `80` |
| wallet-backend | AWS Load Balancer Controller Ingress/public ALB | `wallet/wallet-backend` | ClusterIP `80` |
| woori-backend | AWS Load Balancer Controller Ingress/public ALB | `woori/woori-backend` | ClusterIP `80` |
| Grafana | AWS Load Balancer Controller Ingress/public ALB | `monitoring/kube-prometheus-stack-grafana` | ClusterIP `80` |

backend 외부 `/metrics` path는 Ingress의 ALB fixed-response action으로 `403` 차단합니다. Prometheus는 외부 ALB가 아니라 Kubernetes 내부 ServiceMonitor로 `/metrics`를 scrape합니다.
...
legacy/optional `edge-*` 스택을 개별 apply할 때는 Route53 hosted zone과 public NS 위임을 먼저 검사합니다. 기본 배포에는 사용하지 않습니다.
...
이 명령은 Ingress ALB와 Kubernetes app/DB/monitoring/Argo CD workload를 내립니다. DB PVC와 platform 리소스는 유지하므로 MySQL 데이터는 남지만, EKS control plane, NAT Gateway, node group 비용은 계속 발생합니다.
