...
```text
Terraform은 AWS 인프라와 DNS 기반 리소스를 관리한다.
...
infra/platform
  ...
  AWS Load Balancer Controller IRSA, legacy API Gateway VPC Link/shared internal ALB support
...
infra/edge-frontend
  legacy frontend API Gateway stack입니다.
  기본 apply-all/stop-all/destroy-all lifecycle에서는 사용하지 않습니다.

infra/edge-woori
  legacy woori-backend API Gateway stack입니다.
  기본 apply-all/stop-all/destroy-all lifecycle에서는 사용하지 않습니다.

infra/edge-wallet
  legacy wallet-backend API Gateway stack입니다.
  기본 apply-all/stop-all/destroy-all lifecycle에서는 사용하지 않습니다.

infra/edge-monitoring
  legacy Grafana API Gateway stack입니다.
  기본 apply-all/stop-all/destroy-all lifecycle에서는 사용하지 않습니다.

modules/service-edge
  legacy/optional edge stacks가 공유하는 Terraform module
...
frontend, wallet-backend, woori-backend, Grafana는 public ALB Ingress로 공개합니다.

```text
Browser / Mobile App
  -> frontend.dannis.cloud, wallet-api.dannis.cloud, woori-api.dannis.cloud, grafana.dannis.cloud
...
앱 Ingress manifest가 public ALB를 요청하고 각 앱의 ClusterIP Service를 바라봅니다. legacy `edge-*` Terraform 스택은 기본 lifecycle에서 사용하지 않습니다.

서비스별 계약:

| 서비스 | 진입 방식 | Kubernetes namespace/service | Backend 연결 | Health check |
| --- | --- | --- | --- | --- |
| frontend | AWS Load Balancer Controller Ingress/public ALB | `frontend/frontend` | ClusterIP `80` | `/` |
| woori-backend | AWS Load Balancer Controller Ingress/public ALB | `woori/woori-backend` | ClusterIP `80` | `/api/health` |
| wallet-backend | AWS Load Balancer Controller Ingress/public ALB | `wallet/wallet-backend` | ClusterIP `80` | `/api/health` |
| Grafana | AWS Load Balancer Controller Ingress/public ALB | `monitoring/kube-prometheus-stack-grafana` | ClusterIP `80` | `/api/health` |
...
외부 Ingress 경로의 /metrics는 ALB fixed-response action으로 403 차단합니다.
```
...
```text
apply-all에서 Grafana Ingress를 생성합니다.
Prometheus와 Alertmanager는 계속 비공개입니다.
Grafana는 grafana.dannis.cloud에서 Grafana 로그인으로 보호합니다.
```
...
11. Helm으로 AWS Load Balancer Controller 설치
12. Helm으로 Argo CD 설치
13. Argo CD infra repo credential Secret 적용
14. Argo CD Application manifest 적용
15. monitoring Secret 확인 및 Grafana 준비 대기
16. app Secret 확인 및 app/DB/Ingress 준비 대기
```
...
비용을 더 줄이려면 platform까지 내립니다. `bootstrap/state`와 `infra/dns`는 유지합니다.
...
2. workloads-delete
3. data-delete
4. terraform destroy SERVICE_MODE=platform
```

순서가 중요한 이유:

```text
public ALB는 Ingress가 만들기 때문에 workloads-delete에서 Ingress/namespace를 먼저 지워야 platform VPC 삭제가 막히지 않습니다.
...
kubectl get pods -A
kubectl get applications -n argocd
kubectl -n frontend get ingress frontend
...
```text
platform까지 destroy 후 다시 apply하면 public ALB DNS 이름이 바뀔 수 있습니다.
Ingress가 만든 ALB DNS 이름을 Route53 alias로 연결합니다.
```

## 14. Custom Domain

앱 서브도메인은 Ingress host와 public ALB DNS alias를 사용합니다. ExternalDNS는 설치하지 않으므로 apply 후 ALB 주소를 확인해 Route53 alias를 연결합니다.

| 서비스 | 진입 관리 | 고정 URL |
| --- | --- | --- |
| frontend | Kubernetes Ingress/public ALB | `https://frontend.dannis.cloud` |
...
Legacy/optional API Gateway edge throttling:
...
Legacy service-edge module에서 JWT authorizer는 옵션입니다. issuer/audience를 넣으면 API Gateway 레벨 JWT authorizer가 켜집니다.
...
NAT Gateway는 1개만 사용
app replica는 1개 유지
Grafana public access는 API Gateway가 아니라 ALB Ingress 사용
Prometheus PVC disabled
Grafana persistence disabled
```
...
kubectl get ingress -n wallet wallet-backend
kubectl get ingress -n woori woori-backend
kubectl get endpoints -n wallet wallet-backend
kubectl get endpoints -n woori woori-backend
```
...
Ingress가 아직 public ALB rule을 만들지 않았다.
backend pod가 readiness를 통과하지 못했다.
ALB target group health check가 아직 healthy가 아니다.
```
