...
7. Helm으로 Argo CD 설치
8. Argo CD infra repo credential Secret 적용
9. Argo CD Application manifest 적용
10. monitoring Secret 확인 및 Grafana 준비 대기
11. app Secret 확인 및 app/DB 준비 대기
12. terraform apply SERVICE_MODE=edge-woori
13. terraform apply SERVICE_MODE=edge-wallet
14. ENABLE_GRAFANA_EDGE=yes일 때만 terraform apply SERVICE_MODE=edge-monitoring
```
...
EKS가 아직 존재하는데 kubectl 접근이 안 되면 PVC 삭제를 건너뛰지 않고 실패합니다.
```
