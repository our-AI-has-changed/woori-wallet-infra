...
output "api_gateway_id" {
  description = "HTTP API Gateway ID for the wallet service."
  value       = aws_apigatewayv2_api.this.id
}

output "api_gateway_endpoint" {
  description = "HTTP API Gateway endpoint for the wallet service."
  value       = aws_apigatewayv2_api.this.api_endpoint
}

output "docs_url" {
  description = "Swagger UI URL for the wallet service."
  value       = "${aws_apigatewayv2_api.this.api_endpoint}/docs"
}

output "custom_domain_name" {
  description = "Custom domain name for the wallet service, when enabled."
...
output "api_route" {
  description = "API Gateway route path for the wallet service."
  value       = "$default"
}
